...
def setup_train_data(colorConv, orient, pix_per_cell, cell_per_block, hog_channel):
    masks = []
    images = glob.glob('C:/Users/Vasista/Desktop/THE SYSTEM/STUDY/project/openCV tutorial files/resized1/**/*.png', recursive=True)
    for image in images:
        masks.append(image)
    
    images = glob.glob('C:/Users/Vasista/Desktop/THE SYSTEM/STUDY/project/openCV tutorial files/resized2/**/*.png', recursive=True)
    notmasks = []
    for image in images:
...
cm1 = confusion_matrix(y_test, predictions1, labels=clf.classes_)
